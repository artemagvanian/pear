...
pub enum Usage {
    ...
    Unwind,
    /// Item collected from a compile time function evaluation alloc.
    CTFE,
}
...
impl<'tcx> UsedMonoItem<'tcx> {
    ...
    pub fn instance_if_callable(&self, tcx: TyCtxt<'tcx>) -> Option<Instance<'tcx>> {
        match self.item {
            MonoItem::Fn(instance) => Some(instance),
            MonoItem::Static(def_id) => {
                let static_type = tcx.type_of(def_id).instantiate_identity();
                if static_type.is_fn() {
                    Some(Instance::mono(tcx, def_id))
                } else {
                    None
                }
            }
            MonoItem::GlobalAsm(..) => {
                // TODO: decide what should we do with inline assembly.
                None
            }
        }
    }
...
type MonoItems<'tcx> = Vec<UsedMonoItem<'tcx>>;
...
fn collect_items_rec<'tcx>(
    ...
    match starting_item.item() {
        MonoItem::Fn(instance) => {
            rustc_data_structures::stack::ensure_sufficient_stack(|| {
                collect_used_items(tcx, instance, &mut used_items, attempt_resolving_partial);
            });
        }
        MonoItem::Static(def_id) => {
            let instance = Instance::mono(tcx, def_id);
            let ty = instance.ty(tcx, ty::ParamEnv::reveal_all());
            visit_drop_use(tcx, ty, true, &mut used_items, attempt_resolving_partial);
    ...
    usage_map.record_used(starting_item, &used_items);

    for used_item in used_items {
        if !tcx.is_foreign_item(used_item.item().def_id()) {
            collect_items_rec(
                tcx,
                used_item,
                visited,
            ...
            mir::Rvalue::Cast(
                ...
                match *source_ty.kind() {
                    ty::Closure(def_id, args) => {
                        ...
                        self.output
                            .push(create_fn_mono_item(instance, Usage::FnPtr));
                    }
                    _ => bug!(),
                }
            }
...
fn create_fn_mono_item<'tcx>(instance: Instance<'tcx>, usage: Usage) -> UsedMonoItem<'tcx> {
    ...
    if let ty::Dynamic(trait_ty, ..) = trait_ty.kind() {
        if let Some(principal) = trait_ty.principal() {
            ...
            let methods = entries
                ...
                .map(|item| create_fn_mono_item(item, Usage::VtableItem));
            output.extend(methods);
        }

        // Also add the destructor.
        visit_drop_use(tcx, impl_ty, false, output, attempt_resolving_partial);
    }
}
...
fn collect_alloc<'tcx>(tcx: TyCtxt<'tcx>, alloc_id: AllocId, output: &mut MonoItems<'tcx>) {
    match tcx.global_alloc(alloc_id) {
        GlobalAlloc::Static(def_id) => {
            ...
            trace!("collecting static {:?}", def_id);
            output.push(UsedMonoItem::new(MonoItem::Static(def_id), Usage::CTFE));
        }
        ...
        GlobalAlloc::Function(fn_instance) => {
            ...
            output.push(create_fn_mono_item(fn_instance, Usage::CTFE));
        }
